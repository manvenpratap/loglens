...
- **Search Layout Fix & Thread Selector Dropdown Conversion (v6.9.10 — COMPLETED)**:
  - Aligned scroll-driven scrubber timeline position (`#ll-scrubber`) by updating `SCRUBBER._update` to use the formula `calc(250px + (100% - 250px) * ratio)`, ensuring the scrubber line starts exactly at the timeline track boundary (past the `250px` sticky labels column) and moves within the track space.
